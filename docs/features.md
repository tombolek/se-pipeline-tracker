...
**Similar Deals tab:** When field scoring yields fewer than 3 matches, the tab auto-synthesizes a short **playbook** (win pattern, positioning, lead-with, anticipate blockers) from the top matching KB proof points using Claude. Cached 7 days per opp in `ai_summary_cache` under key `kb-playbook-{oppId}`. Manual refresh via the Refresh button in the playbook card. Issue #111 lever 3.
