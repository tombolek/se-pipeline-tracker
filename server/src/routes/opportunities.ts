...
// GET /opportunities/:id/similar-deals/insights/cached
// Returns cached per-result "why it matches" insights generated by Claude.
// Issue #111 lever 2.
router.get('/:id/similar-deals/insights/cached', auth, async (req: Request, res: Response): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json(err('Invalid opportunity id')); return; }
  try {
    res.json(ok(await getCachedInsights(id)));
  } catch (e) {
...
// POST /opportunities/:id/similar-deals/insights/generate
// Sends top N similar deal candidates to Claude for per-result annotation.
// Cached 7 days. Issue #111 lever 2.
router.post('/:id/similar-deals/insights/generate', auth, write, async (req: Request, res: Response): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json(err('Invalid opportunity id')); return; }
  try {
    ...
    console.error('[similar-deals insights generate] error:', e);
    res.status(500).json(err(`Failed to generate insights: ${(e as Error).message}`));
  }
});
