...
router.post('/login', async (req: Request, res: Response): Promise<void> => {
  ...
  const user = await queryOne<User & { password_hash: string }>(
    `SELECT id, email, name, role, is_admin, is_active, show_qualify, force_password_change, column_prefs, teams, theme,
...
router.post('/change-password', requireAuth as unknown as (req: Request, res: Response, next: () => void) => void, async (req: Request, res: Response): Promise<void> => {
  ...
  const user = await queryOne<User>(
    `UPDATE users SET password_hash = $1, force_password_change = false
     WHERE id = $2
     RETURNING id, email, name, role, is_admin, is_active, show_qualify, force_password_change, column_prefs, teams, theme, created_at, last_login_at`,
