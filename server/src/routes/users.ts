...
router.get('/', auth, mgr, async (_req: Request, res: Response): Promise<void> => {
  const rows = await query<User>(
    `SELECT id, email, name, role, is_active, show_qualify, created_at, last_login_at
...
router.post('/', auth, mgr, async (req: Request, res: Response): Promise<void> => {
  ...
  const user = await queryOne<User>(
    `INSERT INTO users (name, email, role, password_hash)
     VALUES ($1, $2, $3, $4)
     RETURNING id, email, name, role, is_active, show_qualify, created_at, last_login_at`,
...
router.patch('/me/preferences', auth, async (req: Request, res: Response): Promise<void> => {
  ...
  const { show_qualify } = req.body as { show_qualify?: boolean };

  if (typeof show_qualify !== 'boolean') {
    res.status(400).json(err('show_qualify must be a boolean'));
    return;
  }

  const user = await queryOne<User>(
    `UPDATE users SET show_qualify = $1 WHERE id = $2
     RETURNING id, email, name, role, is_active, show_qualify, created_at, last_login_at`,
    [show_qualify, userId]
  );
  res.json(ok(user));
