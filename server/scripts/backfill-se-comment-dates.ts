...
async function run() {
  const rows = await query<OppRow>(
    `SELECT id, se_comments, se_comments_updated_at
     FROM opportunities
  ...
  for (const row of rows) {
    const parsed = parseSeCommentDate(row.se_comments);
    if (!parsed) {
      skippedNoParse++;
      continue;
    }
    ...
    await query(
      `UPDATE opportunities SET se_comments_updated_at = $1 WHERE id = $2`,
      [newTs, row.id]
    );
    updated++;
