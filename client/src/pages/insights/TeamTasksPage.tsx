...
const STATUS_META: Record<string, { label: string; dot: string; col: string; count_bg: string }> = {
  open:        { label: 'Open',        dot: 'bg-blue-400',          col: 'border-t-blue-400',          count_bg: 'bg-blue-50 text-blue-700' },
  in_progress: { label: 'In Progress', dot: 'bg-amber-400',        col: 'border-t-amber-400',        count_bg: 'bg-amber-50 text-amber-700' },
  blocked:     { label: 'Blocked',     dot: 'bg-status-overdue',    col: 'border-t-status-overdue',    count_bg: 'bg-red-50 text-status-overdue' },
...
function dueLabel(d: string | null): { text: string; cls: string } {
  ...
  if (diff < 0) return { text: `${Math.abs(diff)}d overdue`, cls: 'text-status-overdue font-medium' };
  if (diff === 0) return { text: 'Today', cls: 'text-status-warning font-medium' };
  if (diff <= 7) return { text: `${diff}d`, cls: 'text-brand-navy' };
  return { text: new Date(d).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), cls: 'text-brand-navy-70' };
}
...
export default function TeamTasksPage() {
  ...
  if (loading) {
    return <div className="flex items-center justify-center flex-1 text-sm text-brand-navy-70">Loading team tasks...</div>;
  }
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="px-8 pt-5 pb-4 flex-shrink-0 space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-semibold text-brand-navy">Team Tasks</h1>
            <p className="text-xs text-brand-navy-70 mt-0.5">{filtered.length} task{filtered.length !== 1 ? 's' : ''} across your team</p>
          </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-full text-[11px] font-medium transition-colors ${
                  filterDue === f.key
                    ...
                    : 'bg-white border border-brand-navy-30/40 text-brand-navy-70 hover:border-brand-navy-30'
                }`}
              >
                {f.label}
                {f.badge !== undefined && (
                  <span className={`ml-1 px-1.5 py-0.5 rounded-full text-[10px] font-semibold ${
                    filterDue === f.key ? 'bg-white/20 text-white' : 'bg-status-overdue/10 text-status-overdue'
                  }`}>{f.badge}</span>
                )}
              </button>
            ))}
          ...
          <div className="relative ml-auto">
            ...
            <input
              ...
              className="pl-8 pr-3 py-1.5 text-xs rounded-lg border border-brand-navy-30/40 bg-white focus:outline-none focus:border-brand-purple w-48"
            />
          </div>
        </div>
      </div>
      ...
      {/* Content */}
      {filtered.length === 0 ? (
        <div className="flex items-center justify-center flex-1 text-sm text-brand-navy-70">No tasks match the current filters.</div>
      ) : view === 'kanban' ? (
        /* ── Kanban ── */
        <div className="flex-1 overflow-x-auto overflow-y-hidden px-8 pb-6">
          <div className="flex gap-4 h-full min-w-max">
            {STATUS_ORDER.map(status => {
              ...
              return (
                <div key={status} className={`w-[280px] flex flex-col bg-white/60 rounded-2xl border border-brand-navy-30/30 border-t-[3px] ${meta.col}`}>
                  <div className="px-4 py-3 flex items-center gap-2 flex-shrink-0">
                    <span className={`w-2 h-2 rounded-full ${meta.dot}`} />
                    <span className="text-xs font-semibold text-brand-navy">{meta.label}</span>
                    ...
                    {col.map(t => {
                      ...
                      return (
                        <div
                          ...
                          className="bg-white rounded-xl border border-brand-navy-30/30 p-3 cursor-pointer hover:border-brand-purple/40 hover:shadow-sm transition-all"
                        >
                          <p className="text-xs font-medium text-brand-navy leading-snug">{t.title}</p>
                          <p className="text-[11px] text-brand-navy-70 mt-1 truncate">{t.opportunity_name}</p>
                          <div className="flex items-center justify-between mt-2">
                            <div className="flex items-center gap-1.5">
                              {t.assigned_to_name && (
                                <div className="w-5 h-5 rounded-full bg-brand-purple-30 flex items-center justify-center text-[9px] font-semibold text-brand-purple flex-shrink-0">
                                  {t.assigned_to_name.split(' ').map(n => n[0]).join('').slice(0, 2)}
                                </div>
                              )}
                              <span className="text-[10px] text-brand-navy-70">{t.assigned_to_name ?? 'Unassigned'}</span>
                            </div>
                            <span className={`text-[10px] ${due.cls}`}>{due.text}</span>
                          </div>
                          {t.is_next_step && (
                            <span className="inline-block mt-1.5 text-[9px] font-semibold uppercase tracking-wide text-brand-purple bg-brand-purple-30/50 px-1.5 py-0.5 rounded">Next Step</span>
                          )}
                        </div>
                      );
                    })}
                ...
                {sorted.map(t => {
                  ...
                  return (
                    <tr
                      key={t.id}
                      onClick={() => setSelectedOppId(t.opportunity_id)}
                      className={`cursor-pointer hover:bg-brand-purple-30/20 transition-colors ${isOverdue ? 'bg-red-50/30' : ''}`}
                    >
                      <td className="px-4 py-2.5 text-xs text-brand-navy max-w-[260px]">
                        <span className="line-clamp-1">{t.title}</span>
                        {t.is_next_step && (
                          <span className="ml-1.5 text-[9px] font-semibold uppercase tracking-wide text-brand-purple bg-brand-purple-30/50 px-1 py-0.5 rounded">Next Step</span>
                        )}
                      </td>
                      <td className="px-4 py-2.5 text-xs text-brand-navy-70 max-w-[200px] truncate">{t.opportunity_name}</td>
                      <td className="px-4 py-2.5">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-xs text-brand-navy-70">{t.assigned_to_name ?? 'Unassigned'}</span>
                        </div>
                      </td>
                      <td className="px-4 py-2.5">
                        <span className="inline-flex items-center gap-1.5 text-[11px] text-brand-navy-70">
