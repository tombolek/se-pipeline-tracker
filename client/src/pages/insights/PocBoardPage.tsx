...
const COLUMN_COUNT_BG: Record<string, string> = {
  'Identified':    'bg-brand-purple-30/50 dark:bg-accent-purple-soft text-brand-purple',
  'In Deployment': 'bg-sky-50 dark:bg-status-d-info-soft text-sky-700',
  'In Progress':   'bg-amber-50 dark:bg-status-d-warning-soft text-amber-700',
  'Wrapping Up':   'bg-emerald-50 dark:bg-status-d-success-soft text-emerald-700',
};
...
function DaysRemaining({ days, overdue }: { days: number | null; overdue: boolean }) {
  if (overdue) return <span className="text-[10px] font-bold bg-status-overdue/10 dark:bg-status-d-overdue-soft text-status-overdue dark:text-status-d-overdue px-1.5 py-0.5 rounded">OVERDUE</span>;
  if (days === null) return <span className="text-brand-navy-30 dark:text-fg-4">—</span>;
  if (days === 0) return <span className="text-[10px] font-semibold text-status-overdue dark:text-status-d-overdue">Due today</span>;
  if (days <= 7) return <span className="text-[10px] font-semibold text-status-warning dark:text-status-d-warning">{days}d left</span>;
  return <span className="text-[10px] text-brand-navy-70 dark:text-fg-2">{days}d left</span>;
}
  ...
  return (
    <div onClick={onClick} className="bg-white dark:bg-ink-1 rounded-xl border border-brand-navy-30/40 dark:border-ink-border-soft p-3.5 shadow-sm hover:shadow-md transition-shadow cursor-pointer">
      <div className="flex items-start justify-between gap-2 mb-0.5">
        ...
        {opp.is_closed_lost && (
          <span className="text-[9px] font-semibold bg-brand-pink/10 dark:bg-accent-pink-soft text-brand-pink dark:text-accent-pink px-1.5 py-0.5 rounded-full whitespace-nowrap flex-shrink-0">Closed</span>
        )}
      </div>
      ...
      <div className="border-t border-brand-navy-30/30 dark:border-ink-border-soft pt-2 space-y-1.5">
        <div className="flex items-center gap-1.5 text-[11px]">
          <span className="text-brand-navy-30 dark:text-fg-4 font-medium w-9 flex-shrink-0">Start</span>
          <span className="text-brand-navy-70 dark:text-fg-2">{formatDate(opp.poc_start_date) ?? '—'}</span>
        </div>
        <div className="flex items-center gap-1.5 text-[11px]">
          <span className="text-brand-navy-30 dark:text-fg-4 font-medium w-9 flex-shrink-0">End</span>
          <span className={overdue ? 'text-status-overdue dark:text-status-d-overdue font-semibold' : 'text-brand-navy-70 dark:text-fg-2'}>{formatDate(opp.poc_end_date) ?? '—'}</span>
        </div>
        <div className="flex items-center gap-1.5 text-[11px]">
          <span className="text-brand-navy-30 dark:text-fg-4 font-medium w-9 flex-shrink-0">Left</span>
        ...
        <div className="flex items-center justify-between pt-0.5">
          ...
          <div className="flex flex-col items-end gap-0.5 min-w-0">
            {opp.poc_type && <span className="text-[10px] text-brand-navy-30 dark:text-fg-4 truncate max-w-[120px]">{opp.poc_type}</span>}
            {opp.poc_deploy_type && <span className="text-[10px] text-brand-navy-30 dark:text-fg-4 truncate max-w-[120px]">{opp.poc_deploy_type}</span>}
          </div>
        </div>
      </div>
    </div>
...
function PocCardCompact({ opp, expanded, onToggleExpand, onClick }: {
  ...
  return (
    <div className="bg-white dark:bg-ink-1 rounded-xl border border-brand-navy-30/40 dark:border-ink-border-soft shadow-sm hover:shadow-md transition-shadow overflow-hidden">
      ...
      <div className="px-3 pt-2 pb-1.5 cursor-pointer" onClick={onClick}>
        ...
        <div className="flex items-start gap-1.5">
          <p className="text-xs font-semibold text-brand-navy dark:text-fg-1 flex-1 leading-snug">{opp.name}</p>
          <button
            onClick={e => { e.stopPropagation(); onToggleExpand(); }}
            className="flex-shrink-0 mt-0.5 p-0.5 text-brand-navy-30 dark:text-fg-4 hover:text-brand-navy dark:text-fg-1 transition-colors"
        ...
        <div className="flex items-center gap-2 mt-0.5">
          <div className="flex items-center gap-1.5 text-[10px] flex-1 text-brand-navy-70 dark:text-fg-2">
            <span>{formatDate(opp.poc_end_date) ?? '—'}</span>
            <DaysRemaining days={opp.days_remaining} overdue={overdue} />
          </div>
          {opp.se_owner_name && (
            <div className="w-5 h-5 rounded-full bg-brand-purple dark:bg-accent-purple flex-shrink-0 flex items-center justify-center text-[8px] font-bold text-white" title={opp.se_owner_name}>
      ...
      {/* Expanded details */}
      {expanded && (
        <div className="px-3 pb-2.5 border-t border-brand-navy-30/20 dark:border-ink-border-soft pt-2 space-y-1">
          <div className="flex items-center gap-1.5 text-[11px]">
            <span className="text-brand-navy-30 dark:text-fg-4 w-9 flex-shrink-0">Account</span>
            <span className="text-brand-navy-70 dark:text-fg-2 truncate">{opp.account_name ?? '—'}</span>
          </div>
          <div className="flex items-center gap-1.5 text-[11px]">
            <span className="text-brand-navy-30 dark:text-fg-4 w-9 flex-shrink-0">AE</span>
            <span className="text-brand-navy-70 dark:text-fg-2 truncate">{opp.ae_owner_name ?? '—'}</span>
          </div>
          <div className="flex items-center gap-1.5 text-[11px]">
            <span className="text-brand-navy-30 dark:text-fg-4 w-9 flex-shrink-0">Start</span>
            <span className="text-brand-navy-70 dark:text-fg-2">{formatDate(opp.poc_start_date) ?? '—'}</span>
          </div>
          <div className="flex items-center justify-between pt-0.5">
            <span className="text-[11px] text-brand-navy-70 dark:text-fg-2 font-medium">{formatARR(opp.arr)}</span>
...
function KanbanColumn({ title, cards, compact, wide, expandedCards, onToggleExpand, onCardClick }: {
  ...
  return (
    <div className={`flex-shrink-0 flex flex-col h-full bg-white/60 rounded-2xl border border-brand-navy-30/30 dark:border-ink-border-soft border-t-[3px] ${borderTop} ${wide ? 'w-96' : 'w-72'}`}>
      ...
      <div className="flex-1 overflow-y-auto space-y-2 px-3 pb-3">
        {cards.length === 0 ? (
          <div className="text-xs text-brand-navy-30 dark:text-fg-4 text-center py-12 border-2 border-dashed border-brand-navy-30/30 dark:border-ink-border-soft rounded-xl">None</div>
        ) : compact ? (
          cards.map(c => (
            <PocCardCompact
              key={c.id} opp={c}
    ...
    <div className="flex flex-col h-full relative">
      ...
      <div className="px-8 pt-6 pb-4 flex-shrink-0 space-y-4">
        ...
        <div className="flex items-center gap-3">
          <div>
            <h1 className="text-xl font-semibold text-brand-navy dark:text-fg-1">PoC Board</h1>
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <button
              ...
              className={`p-1.5 rounded-lg border transition-colors ${
                hideEmpty
                  ? 'bg-brand-purple/10 dark:bg-accent-purple-soft border-brand-purple text-brand-purple'
                  : 'border-brand-navy-30 text-brand-navy-70 dark:text-fg-2 hover:text-brand-navy'
              }`}
            >
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <select
            ...
            className="text-xs border border-brand-navy-30 rounded-lg px-2.5 py-1.5 text-brand-navy dark:text-fg-1 bg-white dark:bg-ink-1 focus:outline-none focus:border-brand-purple"
          >
          ...
          {filtersActive && (
            <button
              ...
              className="text-xs text-brand-pink dark:text-accent-pink hover:text-brand-pink dark:text-accent-pink-70 font-medium transition-colors"
            >
              Clear filters
            </button>
          )}
