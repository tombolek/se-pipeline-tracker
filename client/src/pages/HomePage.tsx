...
function dueLabel(d: string | null): { text: string; cls: string } {
  ...
  if (diff < 0) return { text: `${Math.abs(diff)}d overdue`, cls: 'text-status-overdue dark:text-status-d-overdue font-semibold' };
  if (diff === 0) return { text: 'Due today', cls: 'text-status-warning dark:text-status-d-warning font-semibold' };
  return { text: `${diff}d`, cls: 'text-brand-navy-70 dark:text-fg-2' };
}
...
// ── Summary Card ────────────────────────────────────────────────────────────
// `tint` is a gradient pair — e.g. `from-status-overdue/5 to-status-overdue/10
// dark:from-ink-1 dark:to-status-d-overdue-soft` — that washes each card with
// a subtle metric-specific tint (matches the light-mode AI Quick Links look).
// Icon chip sits on a neutral backdrop so the colored SVG inside carries the
// accent on its own; spec forbids purple fills on neutral surfaces.
function SummaryCard({ count, label, tint, icon }: { count: number; label: string; tint: string; icon: React.ReactNode }) {
  return (
    <div className={`bg-gradient-to-br ${tint} rounded-2xl border border-brand-navy-30/40 dark:border-ink-border-soft p-4 flex items-center gap-3`}>
      <div className="w-10 h-10 rounded-xl bg-white/80 dark:bg-ink-3 flex items-center justify-center">{icon}</div>
      <div>
        <p className={`text-2xl font-semibold ${count > 0 ? 'text-brand-navy dark:text-fg-1' : 'text-brand-navy-30 dark:text-fg-4'}`}>{count}</p>
        <p className="text-[10px] text-brand-navy-70 dark:text-fg-3 uppercase tracking-wide">{label}</p>
      </div>
    </div>
  );
}
...
function SectionCard({ icon, title, badge, children, footer }: {
  ...
  return (
    <div className="bg-white dark:bg-ink-1 rounded-2xl border border-brand-navy-30/40 dark:border-ink-border-soft overflow-hidden">
      <div className="px-5 py-3.5 border-b border-brand-navy-30/20 dark:border-ink-border-soft flex items-center gap-2">
        {icon}
      ...
      {children}
      {footer && (
        <div className="px-5 py-2.5 border-t border-brand-navy-30/20 dark:border-ink-border-soft">{footer}</div>
      )}
    </div>
  );
}
...
function AllClear({ text }: { text: string }) {
  return (
    <div className="px-5 py-8 text-center">
      <div className="w-10 h-10 mx-auto rounded-full bg-status-success/10 dark:bg-status-d-success-soft flex items-center justify-center mb-2">
        <svg className="w-5 h-5 text-status-success dark:text-status-d-success" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
        </svg>
      </div>
      <p className="text-xs text-brand-navy-70 dark:text-fg-2">{text}</p>
    </div>
  );
}
...
export default function HomePage() {
  ...
  const [drawerInitialTab, setDrawerInitialTab] = useState<'work' | 'timeline' | 'call-prep' | 'demo-prep' | 'similar-deals' | 'tech-discovery' | 'deal-info' | undefined>(undefined);
  ...
  if (loading) {
    return <div className="flex items-center justify-center flex-1 text-sm text-brand-navy-70 dark:text-fg-2">Loading your daily digest...</div>;
  }
  if (!data) {
  ...
  return (
    <div className="flex-1 bg-[#F5F5F7] dark:bg-ink-0 overflow-y-auto">
      <div className="max-w-[1200px] mx-auto px-8 py-6">
        ...
        <div className="grid grid-cols-5 gap-4 mb-6">
          <SummaryCard count={s.overdue} label="Overdue" tint="from-status-overdue/5 to-status-overdue/10 dark:from-ink-1 dark:to-status-d-overdue-soft"
            icon={<svg className="w-5 h-5 text-status-overdue dark:text-status-d-overdue" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>} />
          <SummaryCard count={s.due_today} label="Due Today" tint="from-status-warning/5 to-status-warning/10 dark:from-ink-1 dark:to-status-d-warning-soft"
            icon={<svg className="w-5 h-5 text-status-warning dark:text-status-d-warning" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/></svg>} />
          <SummaryCard count={s.poc_alerts} label="PoC Alerts" tint="from-brand-purple/5 to-brand-purple/10 dark:from-ink-1 dark:to-accent-purple-soft"
            icon={<svg className="w-5 h-5 text-brand-purple dark:text-accent-purple" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>} />
          <SummaryCard count={s.closed_lost_unread} label="New Closed Lost" tint="from-brand-pink/5 to-brand-pink/10 dark:from-ink-1 dark:to-accent-pink-soft"
            icon={<svg className="w-5 h-5 text-brand-pink dark:text-accent-pink" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"/></svg>} />
          <SummaryCard count={s.hygiene_issues} label="Needs Attention" tint="from-status-warning/5 to-status-warning/10 dark:from-ink-1 dark:to-status-d-warning-soft"
          ...
          {([
            {
              ...
              icon: (
                <svg className="w-5 h-5 text-brand-purple dark:text-accent-purple" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z" />
                </svg>
              ),
              gradient: 'from-brand-purple/5 to-brand-purple/10 dark:from-ink-1 dark:to-accent-purple-soft',
            },
            {
              ...
              icon: (
                <svg className="w-5 h-5 text-status-success dark:text-status-d-success" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 3v11.25A2.25 2.25 0 006 16.5h2.25M3.75 3h-1.5m1.5 0h16.5m0 0h1.5m-1.5 0v11.25A2.25 2.25 0 0118 16.5h-2.25m-7.5 0h7.5m-7.5 0l-1 3m8.5-3l1 3m0 0l.5 1.5m-.5-1.5h-9.5m0 0l-.5 1.5M9 11.25v1.5M12 9v3.75m3-6v6" />
                </svg>
              ),
              gradient: 'from-status-success/5 to-status-success/10 dark:from-ink-1 dark:to-status-d-success-soft',
            },
            {
              ...
              icon: (
                <svg className="w-5 h-5 text-status-info dark:text-status-d-info" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
                </svg>
              ),
              gradient: 'from-status-info/5 to-status-info/10 dark:from-ink-1 dark:to-status-d-info-soft',
            },
            {
              ...
              icon: (
                <svg className="w-5 h-5 text-status-warning dark:text-status-d-warning" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.455 2.456L21.75 6l-1.036.259a3.375 3.375 0 00-2.455 2.456z" />
                </svg>
              ),
              gradient: 'from-status-warning/5 to-status-warning/10 dark:from-ink-1 dark:to-status-d-warning-soft',
            },
          ]).map(item => (
            <div key={item.key} className="relative">
              <button
                onClick={() => { setAiPickerOpen(aiPickerOpen === item.key ? null : item.key); setAiSearch(''); }}
                className={`w-full bg-gradient-to-br ${item.gradient} rounded-2xl border border-brand-navy-30/40 dark:border-ink-border-soft p-4 text-left hover:border-brand-purple/30 dark:hover:border-accent-purple/30 hover:shadow-sm dark:hover:shadow-none transition-all group ${
                  aiPickerOpen === item.key ? 'border-brand-purple/40 dark:border-accent-purple/40 shadow-sm' : ''
                }`}
              >
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-white/80 dark:bg-ink-3 flex items-center justify-center shadow-sm dark:shadow-none">
                    {item.icon}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-brand-navy dark:text-fg-1">{item.label}</p>
                    <p className="text-[10px] text-brand-navy-70 dark:text-fg-2 mt-0.5 leading-relaxed">{item.desc}</p>
                  </div>
                  <svg className="w-4 h-4 text-brand-navy-30 dark:text-fg-4 group-hover:text-brand-purple dark:text-accent-purple transition-colors flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              {aiPickerOpen === item.key && (
                <div className="absolute left-0 right-0 top-full mt-1 z-50 bg-white dark:bg-ink-2 border border-brand-navy-30 dark:border-ink-border rounded-xl shadow-lg overflow-hidden">
                  <div className="p-2 border-b border-brand-navy-30/20 dark:border-ink-border-soft">
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-8 pr-3 py-1.5 text-xs border border-brand-navy-30/60 dark:border-ink-border rounded-lg bg-white dark:bg-ink-2 text-brand-navy dark:text-fg-1 focus:outline-none focus:border-brand-purple dark:focus:border-accent-purple focus:ring-1 focus:ring-brand-purple/30 placeholder:text-brand-navy-30 dark:placeholder:text-fg-4"
                      />
                    </div>
                  </div>
                  <div className="max-h-[220px] overflow-y-auto">
                    {filteredAiOpps.length === 0 ? (
                      <p className="px-3 py-4 text-xs text-brand-navy-30 dark:text-fg-4 text-center">No opportunities found</p>
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          <p className="text-brand-navy dark:text-fg-1 font-medium truncate">{o.name}</p>
                          <p className="text-brand-navy-70 dark:text-fg-2 text-[10px] truncate">{o.account_name} &middot; {o.stage}</p>
                        </div>
                        {o.arr != null && (
                          <span className="text-[10px] text-brand-navy-70 dark:text-fg-2 flex-shrink-0">
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="space-y-6">
            ...
            <SectionCard
              ...
              title="Today's Tasks"
              badge={data.tasks.length > 0 ? <span className="text-[10px] font-semibold bg-status-overdue/10 dark:bg-status-d-overdue-soft text-status-overdue dark:text-status-d-overdue px-1.5 py-0.5 rounded-full">{data.tasks.length}</span> : undefined}
              footer={<button onClick={() => navigate('/my-tasks')} className="text-[11px] font-medium text-brand-purple dark:text-accent-purple hover:text-brand-purple-70 dark:text-accent-purple transition-colors">View all my tasks &rarr;</button>}
            >
              {data.tasks.length === 0 ? (
                ...
                <div className="divide-y divide-brand-navy-30/15 dark:divide-ink-border-soft">
                  {data.tasks.slice(0, 8).map(t => {
                    ...
                    const dotColor = due.text.includes('overdue') ? 'bg-status-overdue dark:bg-status-d-overdue ring-status-overdue/20 dark:ring-0'
                      : due.text === 'Due today' ? 'bg-status-warning dark:bg-status-d-warning ring-status-warning/20 dark:ring-0'
                      : 'bg-blue-400 dark:bg-status-d-info ring-blue-400/20 dark:ring-0';
                    return (
                      <div key={t.id} onClick={() => openOpp(t.opportunity_id)} className="px-5 py-3 hover:bg-brand-purple-30/20 dark:hover:bg-accent-purple-soft cursor-pointer transition-colors">
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              <span className="text-[10px] text-brand-navy-70 dark:text-fg-2 truncate">{t.opportunity_name}</span>
                              <span className={`text-[10px] ${due.cls}`}>{due.text}</span>
                            </div>
                          </div>
                          {t.status === 'blocked' && (
                            <span className="text-[9px] font-semibold uppercase tracking-wide text-status-overdue dark:text-status-d-overdue bg-status-overdue/10 dark:bg-status-d-overdue-soft px-1.5 py-0.5 rounded flex-shrink-0">Blocked</span>
                          )}
                          {t.is_next_step && (
                            <span className="text-[9px] font-semibold uppercase tracking-wide text-brand-purple dark:text-accent-purple bg-brand-purple-30/50 dark:bg-accent-purple-soft px-1.5 py-0.5 rounded flex-shrink-0">Next Step</span>
                          )}
                        </div>
                      </div>
                    );
            ...
            {/* Upcoming This Week */}
            <SectionCard
              icon={<svg className="w-4 h-4 text-brand-navy-70 dark:text-fg-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>}
              title="Upcoming This Week"
              footer={<button onClick={() => navigate('/calendar')} className="text-[11px] font-medium text-brand-purple dark:text-accent-purple hover:text-brand-purple-70 dark:text-accent-purple transition-colors">Open calendar &rarr;</button>}
            >
                ...
                <div className="divide-y divide-brand-navy-30/15 dark:divide-ink-border-soft">
                  {data.upcoming.map((ev, i) => {
                    const typeBadge = ev.event_type === 'poc_end'
                      ? { text: 'PoC', cls: 'text-status-overdue dark:text-status-d-overdue bg-status-overdue/10 dark:bg-status-d-overdue-soft' }
                      : ev.event_type === 'rfx_submission'
                      ? { text: 'RFx', cls: 'text-status-warning dark:text-status-d-warning bg-status-warning/10 dark:bg-status-d-warning-soft' }
                      : ev.is_next_step
                      ? { text: 'Next Step', cls: 'text-brand-purple dark:text-accent-purple bg-brand-purple-30/50 dark:bg-accent-purple-soft' }
                      : null;
                    return (
                      <div key={i} onClick={() => openOpp(ev.opportunity_id)} className="px-5 py-3 hover:bg-brand-purple-30/20 dark:hover:bg-accent-purple-soft cursor-pointer transition-colors">
                        <div className="flex items-center gap-3">
                          <span className="text-[10px] font-semibold text-brand-navy-70 dark:text-fg-2 w-12 flex-shrink-0">{formatEventDate(ev.event_date)}</span>
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-medium text-brand-navy dark:text-fg-1">{ev.label}</p>
                            <span className="text-[10px] text-brand-navy-70 dark:text-fg-2">{ev.opportunity_name}</span>
                          </div>
                          {typeBadge && (
            ...
            {/* Stale Deals */}
            <SectionCard
              icon={<svg className="w-4 h-4 text-brand-navy-30 dark:text-fg-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>}
              title="Stale Deals"
              badge={<span className="text-[10px] text-brand-navy-70 dark:text-fg-2">No activity in 21+ days</span>}
            >
              {data.stale_deals.length === 0 ? (
                ...
                <div className="divide-y divide-brand-navy-30/15 dark:divide-ink-border-soft">
                  {data.stale_deals.map(d => {
                    ...
                    return (
                      <div key={d.id} onClick={() => openOpp(d.id)} className="px-5 py-3 hover:bg-brand-purple-30/20 dark:hover:bg-accent-purple-soft cursor-pointer transition-colors">
                        <div className="flex items-start gap-3">
                          <span className="mt-1 w-2 h-2 rounded-full bg-brand-navy-30 flex-shrink-0 ring-2 ring-brand-navy-30/20" />
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-medium text-brand-navy dark:text-fg-1 leading-snug">{d.name}</p>
                            <div className="flex items-center gap-2 mt-1">
                              <span className="text-[10px] text-brand-navy-70 dark:text-fg-2">{d.stage}</span>
                              {d.arr != null && <><span className="text-brand-navy-30 dark:text-fg-4">&middot;</span><span className="text-[10px] text-brand-navy-70 dark:text-fg-2">{formatARR(d.arr)}</span></>}
                              <span className="text-brand-navy-30 dark:text-fg-4">&middot;</span>
                              <span className="text-[10px] text-status-overdue dark:text-status-d-overdue font-medium">{daysSince ? `${daysSince}d silent` : 'Never updated'}</span>
                            </div>
                          </div>
                        </div>
                      </div>
          ...
          <div className="space-y-6">
            ...
            {/* Recent Activity */}
            <SectionCard
              icon={<svg className="w-4 h-4 text-status-info dark:text-status-d-info" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>}
              ...
              {data.recent_activity.length === 0 ? (
                ...
                <div className="divide-y divide-brand-navy-30/15 dark:divide-ink-border-soft">
                  {data.recent_activity.slice(0, 6).map((a, i) => {
                    ...
                    return (
                      <div key={i} onClick={() => openOpp(a.opportunity_id)} className="px-5 py-3 hover:bg-brand-purple-30/20 dark:hover:bg-accent-purple-soft cursor-pointer transition-colors">
                        <div className="flex items-start gap-3">
                          {a.activity_type === 'note' && (
                            <div className="mt-0.5 w-6 h-6 rounded-full bg-brand-purple dark:bg-accent-purple flex items-center justify-center text-[9px] font-bold text-white flex-shrink-0">{initials}</div>
                          )}
                          {a.activity_type === 'stage_change' && (
                            <div className="mt-0.5 w-6 h-6 rounded-full bg-brand-purple-30 dark:bg-accent-purple-soft flex items-center justify-center text-[9px] font-bold text-brand-purple dark:text-accent-purple flex-shrink-0">SF</div>
                          )}
                          {a.activity_type === 'manager_comment' && (
                            <div className="mt-0.5 w-6 h-6 rounded-full bg-brand-pink/20 flex items-center justify-center text-[9px] font-bold text-brand-pink dark:text-accent-pink flex-shrink-0">MG</div>
                          )}
                          <div className="flex-1 min-w-0">
                            {a.activity_type === 'note' && (
                              <>
                                <p className="text-xs text-brand-navy dark:text-fg-1 leading-snug"><span className="font-medium">{a.actor_name}</span> added a note on <span className="font-medium">{a.opportunity_name}</span></p>
                                {a.detail && <p className="text-[11px] text-brand-navy-70 dark:text-fg-2 mt-1 line-clamp-2 italic">&ldquo;{a.detail}&rdquo;</p>}
                              </>
                            )}
                            {a.activity_type === 'stage_change' && (
                              <p className="text-xs text-brand-navy dark:text-fg-1 leading-snug"><span className="font-medium">{a.opportunity_name}</span> moved from <span className="font-medium">{a.extra}</span> &rarr; <span className="font-medium">{a.detail}</span></p>
                            )}
                            {a.activity_type === 'manager_comment' && (
                              <>
                                <p className="text-xs text-brand-navy dark:text-fg-1 leading-snug"><span className="font-medium">Manager</span> updated comments on <span className="font-medium">{a.opportunity_name}</span></p>
                                {a.detail && <p className="text-[11px] text-brand-navy-70 dark:text-fg-2 mt-1 line-clamp-2 italic">&ldquo;{a.detail}&rdquo;</p>}
                              </>
                            )}
                            <span className="text-[10px] text-brand-navy-70 dark:text-fg-2 mt-0.5 block">{timeAgo(a.activity_at)}</span>
                          </div>
                        </div>
                      </div>
                    );
            ...
            {mentions.length > 0 && (
              <SectionCard
                icon={<svg className="w-4 h-4 text-brand-purple dark:text-accent-purple" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207"/></svg>}
                title="Mentions"
                badge={mentionsUnread > 0
                  ? <span className="text-[10px] font-semibold bg-brand-purple/10 dark:bg-accent-purple-soft text-brand-purple dark:text-accent-purple px-1.5 py-0.5 rounded-full">{mentionsUnread} new</span>
                  : <span className="text-[10px] text-brand-navy-70 dark:text-fg-2">When teammates @-mention you in notes</span>}
              >
                <div className="divide-y divide-brand-navy-30/15 dark:divide-ink-border-soft">
                  {mentions.slice(0, 6).map(m => {
                    ...
                    return (
                      <div
                        ...
                        className={`px-5 py-3 hover:bg-brand-purple-30/20 dark:hover:bg-accent-purple-soft cursor-pointer transition-colors ${unread ? 'bg-brand-purple/[0.03] dark:bg-accent-purple-soft' : ''}`}
                      >
                        <div className="flex items-start gap-3">
                          <div className="mt-0.5 w-6 h-6 rounded-full bg-brand-purple dark:bg-accent-purple flex items-center justify-center text-[9px] font-bold text-white flex-shrink-0">
                            {initials}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-xs text-brand-navy dark:text-fg-1 leading-snug">
                              <span className="font-medium">{m.author_name}</span> mentioned you on{' '}
                              <span className="font-medium">{m.opportunity_name}</span>
                            </p>
                            <p className="text-[11px] text-brand-navy-70 dark:text-fg-2 mt-1 line-clamp-2 italic">&ldquo;{m.content}&rdquo;</p>
                            <span className="text-[10px] text-brand-navy-70 dark:text-fg-2 mt-0.5 block">{timeAgo(m.created_at)}</span>
                          </div>
                          {unread && <span className="mt-1 w-2 h-2 rounded-full bg-brand-purple dark:bg-accent-purple flex-shrink-0" />}
                        </div>
                      </div>
                    );
                  })}
            ...
            <SectionCard
              ...
              badge={data.hygiene.length > 0 ? <span className="text-[10px] font-semibold bg-status-warning/10 dark:bg-status-d-warning-soft text-status-warning dark:text-status-d-warning px-1.5 py-0.5 rounded-full">{data.hygiene.length} {data.hygiene.length === 1 ? 'deal' : 'deals'}</span> : <span className="text-[10px] text-brand-navy-70 dark:text-fg-2">SE data hygiene</span>}
            >
              {data.hygiene.length === 0 ? (
                ...
                <div className="divide-y divide-brand-navy-30/15 dark:divide-ink-border-soft">
                  {data.hygiene.slice(0, 6).map(h => (
                    <div key={h.id} onClick={() => openOpp(h.id)} className="px-5 py-3 hover:bg-brand-purple-30/20 dark:hover:bg-accent-purple-soft cursor-pointer transition-colors">
                      <div className="flex items-start gap-3">
                        <span className={`mt-1 w-2 h-2 rounded-full flex-shrink-0 ring-2 ${
                          h.flags.some(f => f.includes('overdue') || f.includes('should be')) ? 'bg-status-overdue dark:bg-status-d-overdue ring-status-overdue/20 dark:ring-0' : 'bg-status-warning dark:bg-status-d-warning ring-status-warning/20 dark:ring-0'
                        }`} />
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-medium text-brand-navy dark:text-fg-1 leading-snug">{h.name}</p>
                          <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                            <span className="text-[10px] text-brand-navy-70 dark:text-fg-2">{h.stage}{h.arr != null ? ` · ${formatARR(h.arr)}` : ''}</span>
                            {h.flags.map((f, i) => (
