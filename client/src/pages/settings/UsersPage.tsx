...
import { listUsers, createUser, updateUser, deleteUser, resetUserPassword, listTeams } from '../../api/users';
...
function ResetPasswordRow({ user, onDone }: { user: User; onDone: () => void }) {
  ...
  if (success) {
    return (
      <td colSpan={8} className="px-4 py-3 bg-status-success/5">
  ...
  return (
    <td colSpan={8} className="px-4 py-2 bg-brand-purple/5">
        ...
        <div className="bg-white rounded-2xl border border-brand-navy-30/40 overflow-hidden">
          <table className="w-full">
            <thead className="border-b border-brand-navy-30/40">
              <tr>
                {['Name', 'Email', 'Role', 'Territory', 'Reports To', 'Last Login', 'Status', 'Actions'].map(h => (
