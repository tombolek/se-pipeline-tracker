...
import {
  getDeployStatus, triggerDeploy, getDeployLog,
  type DeployStatus, type DeployLogEntry,
} from '../../api/deploy';
...
export default function DeployPage() {
  ...
  // Load initial status + check if a deploy is already running
  useEffect(() => {
    setLoadingStatus(true);
    getDeployStatus()
      .then(async (s) => {
        setStatus(s);
        // If a deploy is in progress, start polling the latest log
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="bg-white rounded-lg border border-brand-navy-30 p-5 space-y-4">
        ...
        {statusError && (
          <p className="text-sm text-status-overdue">Failed to load status: {statusError}</p>
        )}
          ...
          <>
            <div className="grid grid-cols-2 gap-x-8 gap-y-2 text-sm">
              ...
              <div>
                ...
                {isRunning ? (
                  <span className="inline-flex items-center gap-1.5 text-status-warning font-medium">
      ...
      {activeLog && (
        <div className="bg-white rounded-lg border border-brand-navy-30 overflow-hidden">
          ...
          <div className="bg-brand-navy rounded-b-lg p-4 font-mono text-xs text-white/80 max-h-96 overflow-y-auto leading-relaxed">
              ...
              activeLog.log.map((line, i) => (
                <div
                  ...
                  className={
                    ...
                    line.includes('complete') || line.includes('success') ? 'text-status-success' :
                    'text-white/80'
                  }
                >
                  {line}
            ...
            {isRunning && (
              <div className="text-white/40 mt-1 animate-pulse">▌</div>
            )}
            <div ref={logEndRef} />
          </div>
        </div>
