...
function KindBadge({ kind }: { kind: TemplateKind }) {
  if (kind === 'task_pack') {
    return <span className="px-2 py-0.5 rounded-full text-[11px] font-medium bg-brand-purple-30 text-brand-navy">Task pack</span>;
  }
  return <span className="px-2 py-0.5 rounded-full text-[11px] font-medium bg-emerald-50 text-status-success">Note</span>;
}
...
function TemplateEditor({ initial, onClose, onSaved }: EditorProps) {
  ...
  return (
    <div className="fixed inset-0 bg-brand-navy/30 backdrop-blur-[2px] z-40 flex items-center justify-center p-6">
      <form onSubmit={save} className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="px-5 py-4 border-b border-brand-navy-30/40 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-brand-navy">{initial ? 'Edit' : 'New'} template</h3>
          <button type="button" onClick={onClose} className="text-brand-navy-70 hover:text-brand-navy">
        ...
        <div className="px-5 py-4 space-y-4">
          {error && <p className="text-xs text-status-overdue bg-red-50 rounded px-2 py-1.5">{error}</p>}

          <div className="flex items-center gap-3">
            <label className="text-xs font-semibold text-brand-navy-70 uppercase tracking-wide">Kind</label>
              ...
              {(['task_pack', 'note'] as TemplateKind[]).map(k => (
                <button
                  ...
                  className={`px-3 py-1 rounded text-xs font-medium transition-colors ${
                    kind === k ? 'bg-white text-brand-navy shadow-sm' : 'text-brand-navy-70 hover:text-brand-navy'
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-semibold text-brand-navy-70 uppercase tracking-wide block mb-1">Name</label>
              <input
                value={name}
            ...
            <div>
              <label className="text-xs font-semibold text-brand-navy-70 uppercase tracking-wide block mb-1">Stage (optional)</label>
              <select
                value={stage}
          ...
          <div>
            <label className="text-xs font-semibold text-brand-navy-70 uppercase tracking-wide block mb-1">Description</label>
            <input
              value={description}
          ...
          {kind === 'note' ? (
            <div>
              <label className="text-xs font-semibold text-brand-navy-70 uppercase tracking-wide block mb-1">Note body</label>
              <textarea
                value={body}
                onChange={e => setBody(e.target.value)}
                rows={10}
                className="w-full px-3 py-2 rounded border border-brand-navy-30 text-sm text-brand-navy font-mono focus:outline-none focus:ring-2 focus:ring-brand-purple"
                placeholder={`e.g.\n## Call Recap\n\n**Attendees:**\n\n**Discussion:**\n\n**Next Steps:**`}
              />
              <p className="text-[11px] text-brand-navy-70 mt-1">Free-form text. Applied as a new note on the opportunity.</p>
            </div>
          ) : (
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-xs font-semibold text-brand-navy-70 uppercase tracking-wide">Tasks ({items.length})</label>
                <button type="button" onClick={addItem} className="text-xs text-brand-purple hover:text-brand-navy font-medium">+ Add task</button>
              </div>
              <div className="space-y-2">
                {items.map((it, i) => (
                  <div key={i} className="p-3 rounded-lg border border-brand-navy-30 bg-gray-50 space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      />
                      {items.length > 1 && (
                        <button type="button" onClick={() => removeItem(i)} className="text-brand-navy-30 hover:text-status-overdue" title="Remove">
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                          </svg>
                        </button>
        ...
        <div className="px-5 py-3 border-t border-brand-navy-30/40 flex justify-end gap-2">
          <button type="button" onClick={onClose} className="px-3 py-1.5 text-xs text-brand-navy-70 hover:text-brand-navy">Cancel</button>
          <button type="submit" disabled={saving} className="px-4 py-1.5 bg-brand-purple text-white text-xs font-medium rounded hover:bg-brand-purple-70 disabled:opacity-50">
            {saving ? 'Saving…' : 'Save'}
          </button>
        </div>
      </form>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-xl font-semibold text-brand-navy">Templates</h1>
        ...
        <button
          ...
          className="px-3 py-1.5 bg-brand-purple text-white text-xs font-medium rounded hover:bg-brand-purple-70"
        >
          + New template
        </button>
      </div>
      ...
      {loading ? (
        <p className="text-xs text-brand-navy-70">Loading…</p>
      ) : templates.length === 0 ? (
        <div className="bg-white rounded-2xl border border-brand-navy-30/40 px-6 py-10 text-center">
          <p className="text-sm text-brand-navy-70">No templates yet.</p>
          <p className="text-xs text-brand-navy-30 mt-1">Create task packs (e.g. "PoC Kickoff") or note templates (e.g. "Call Recap") to help SEs move fast on new opportunities.</p>
        </div>
      ) : (
  ...
  return (
    <div className="bg-white rounded-2xl border border-brand-navy-30/40 overflow-hidden">
      <div className="px-5 py-3 border-b border-brand-navy-30/40">
        <h2 className="text-sm font-semibold text-brand-navy">{title} <span className="text-brand-navy-70 font-normal">({items.length})</span></h2>
      </div>
