  ...
  return (
    <form onSubmit={submit} className="flex-1 space-y-1.5">
      <input
        ...
        className="w-full px-2 py-1 rounded border border-brand-navy-30 text-sm text-brand-navy dark:text-fg-1 focus:outline-none focus:ring-1 focus:ring-brand-purple"
      />
      <input
        value={description}
        onChange={e => setDescription(e.target.value)}
        placeholder="Description (optional)"
        className="w-full px-2 py-1 rounded border border-brand-navy-30 text-xs text-brand-navy dark:text-fg-1 placeholder:text-brand-navy-30 dark:text-fg-4 focus:outline-none focus:ring-1 focus:ring-brand-purple"
      />
      <div className="flex items-center gap-2">
        <input
          ...
          className="px-2 py-0.5 rounded border border-brand-navy-30 text-xs text-brand-navy dark:text-fg-1 focus:outline-none focus:ring-1 focus:ring-brand-purple"
        />
        <button type="submit" disabled={saving || !title.trim()} className="px-2.5 py-0.5 bg-brand-purple dark:bg-accent-purple text-white text-xs font-medium rounded hover:bg-brand-purple-70 dark:hover:opacity-90 disabled:opacity-50 transition-colors">
          {saving ? 'Saving…' : 'Save'}
        </button>
        <button type="button" onClick={onCancel} className="px-2 py-0.5 text-xs text-brand-navy-70 dark:text-fg-2 hover:text-brand-navy dark:text-fg-1 transition-colors">
          Cancel
        </button>
      </div>
    </form>
...
export function TaskRow({
  ...
  if (editing) {
    return (
      <div className="flex items-start gap-3 py-2.5 border-b border-brand-navy-30/40 dark:border-ink-border-soft last:border-0">
  ...
  return (
    <div className={`group flex items-start gap-3 py-2.5 border-b border-brand-navy-30/40 dark:border-ink-border-soft last:border-0 ${task.status === 'done' ? 'opacity-60' : ''}`}>
      <button
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-sm text-brand-navy dark:text-fg-1 leading-tight ${task.status === 'done' ? 'line-through' : ''}`}>{task.title}</p>
        {task.description && <p className="text-xs text-brand-navy-70 dark:text-fg-2 mt-0.5 line-clamp-2">{task.description}</p>}
        <div className="flex items-center gap-2 mt-1 flex-wrap">
          {task.assigned_to_name && (
            <span className="text-[11px] text-brand-navy-70 dark:text-fg-2">{task.assigned_to_name}</span>
          )}
          {task.due_date && (
      ...
      {!readOnly && (
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity mt-0.5">
          <button
            ...
            className="text-brand-navy-30 dark:text-fg-4 hover:text-brand-purple dark:text-accent-purple"
            title="Edit task"
          >
          ...
          <button
            ...
            className="text-brand-navy-30 dark:text-fg-4 hover:text-status-overdue dark:text-status-d-overdue"
            title="Delete task"
          >
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
  ...
  return (
    <form onSubmit={submit} className="mt-2 p-3 bg-gray-50 dark:bg-ink-0 rounded-lg border border-brand-navy-30 space-y-2">
      ...
      <div className="flex gap-2">
        <button type="submit" disabled={saving || !title.trim()} className="px-3 py-1 bg-brand-purple dark:bg-accent-purple text-white text-xs font-medium rounded hover:bg-brand-purple-70 dark:hover:opacity-90 disabled:opacity-50 transition-colors">
