...
export default function ContributorsStrip({ oppId, ownerId, contributors, readOnly, onChange }: Props) {
  ...
  return (
    <div className="flex items-center gap-1.5 flex-wrap" ref={ref}>
      <span className="text-[10px] font-semibold uppercase tracking-wide text-brand-navy-70">Contributors</span>
      {contributors.length === 0 && <span className="text-[11px] text-brand-navy-30">None</span>}
      {contributors.map(c => {
        const canRemove = canManage || (canToggleSelf && c.id === user?.id);
        return (
          <span key={c.id} className="group inline-flex items-center gap-1 bg-brand-purple-30/60 text-brand-navy text-[11px] font-medium px-2 py-0.5 rounded-full">
            {c.name}
            {canRemove && (
              <button
                type="button"
                onClick={() => remove(c.id)}
                disabled={loading}
                className="w-3 h-3 rounded-full text-brand-navy-70 hover:text-status-overdue transition-colors"
                title={`Remove ${c.name}`}
              >
      ...
      {(canManage || canToggleSelf) && (
        <div className="relative inline-block">
          <button
            ...
            onClick={() => setOpen(o => !o)}
            disabled={loading}
            className="text-[11px] text-brand-purple hover:text-brand-navy font-medium disabled:opacity-50"
          >
            + Add
          </button>

          {open && (
            <div className="absolute left-0 top-5 z-30 w-64 bg-white rounded-lg border border-brand-navy-30 shadow-xl overflow-hidden">
