...
function sectionColor(kind: string): string {
  ...
  if (k === 'added')   return 'text-status-success bg-emerald-50';
  if (k === 'changed') return 'text-brand-purple bg-brand-purple-30';
  if (k === 'fixed')   return 'text-status-info bg-sky-50';
  if (k === 'removed' || k === 'deprecated') return 'text-status-overdue bg-red-50';
  if (k === 'security') return 'text-status-warning bg-amber-50';
  return 'text-brand-navy bg-gray-100';
}
...
function renderInline(text: string): React.ReactNode {
  ...
  while ((match = re.exec(text)) !== null) {
    ...
    if (m.startsWith('**')) {
      ...
      parts.push(<code key={i++} className="px-1 py-0.5 rounded bg-gray-100 text-[11px] font-mono">{m.slice(1, -1)}</code>);
    }
  ...
  return (
    <div className="bg-white rounded-2xl border border-brand-navy-30/40 overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-brand-navy-30/40">
        <span className="text-sm font-semibold text-brand-navy">{formatDate(entry.date)}</span>
        {isNew && (
        ...
        {entry.sections.map((s, i) => (
          <div key={i}>
            ...
            <ul className="mt-2 space-y-1.5 ml-0.5">
              {s.bullets.map((b, j) => (
                <li key={j} className="text-xs text-brand-navy-70 leading-relaxed flex gap-2">
                  <span className="text-brand-navy-30 flex-shrink-0 mt-0.5">•</span>
...
export default function ChangelogModal({ open, onClose }: Props) {
  ...
  return createPortal(
    <>
      ...
      <div className="fixed top-0 right-0 h-full w-[560px] max-w-[95vw] bg-gray-50 shadow-2xl z-50 flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 bg-white border-b border-brand-navy-30/40 flex-shrink-0">
          <div>
            <h2 className="text-base font-semibold text-brand-navy">What's New</h2>
            <p className="text-[11px] text-brand-navy-70 mt-0.5">Recent changes to Pipeline Tracker</p>
          </div>
          <button
            onClick={onClose}
