...
export default function OutOfTerritoryBanner({
  ...
  return (
    <div className="bg-status-info/10 border border-status-info/30 rounded-xl text-xs text-brand-navy flex-shrink-0">
      <div className="flex items-center gap-2 px-3 py-2">
        ...
        {items && items.length > 0 && (
          <button
            ...
            className="ml-2 flex items-center gap-1 text-status-info font-semibold hover:opacity-80 transition-opacity flex-shrink-0"
          >
        ...
        <div className="border-t border-status-info/20 px-3 py-2 space-y-1.5">
          {items.map(item => (
            <div key={item.id} className="flex items-center gap-2">
              <span className="font-medium text-brand-navy truncate">{item.name}</span>
              <span className="flex-shrink-0 px-1.5 py-px rounded-full bg-status-info/15 text-status-info text-[10px] font-semibold">
                {item.team}
              </span>
            </div>
          ))}
